...
THREE.EchoSonarSALDO = function ( width, height, _params ) {

    THREE.Pass.call( this );

    cameracount = _params.cameracount;
    ...
    this.depthDisplay  = {
        uniforms: {
            ...
            "timeLast": { value: 0.0 },
            "vfov": { value: 0.0 },
            "sunDir": { value: new THREE.Vector3( 0.70707, 0.70707, 0 ) }
        },
        vertexShader: [
            "varying vec2 vUv;",
            "void main() {",
        ...
        fragmentShader: [
            ...
            "void main() {",
                ...
                "float vpixelStep = mod(vUv.y * vfov, stepAzimuth);",
                "float htest = 0.1;",
                "if ((pixelSkip >= 0.0) && (pixelSkip <= htest)) {", 
                "if ((vpixelStep >= 0.0) && (vpixelStep <= htest)) {", 
    ...
    this.lidarCamera = new THREE.PerspectiveCamera( this.params.vfov , this.params.vfov / this.params.vertAzimuth, this.params.cameraNear, this.params.cameraFar );
...
THREE.EchoSonarSALDO.prototype = Object.assign( Object.create( THREE.Pass.prototype ), {
	constructor: THREE.EchoSonarSALDO,

	render: function ( renderer, writeBuffer, readBuffer, delta, maskActive ) {

        var camwidth = 360/this.cameracount;
        var cam = 0;
        var rot = 0;

        this.renderTarget.scissorTest = true;
        // HACK: Hide ocean
        var saveocean = ocean.visible;
        var savegridhelper = gridhelper.visible;
        if(ocean) ocean.visible = false;
        if(gridhelper) gridhelper.visible = false;

if( this.cameracount > 0 ) {
            //        this.lidarCamera.lookAt(camera.position.x + 1, camera.position.y, camera.position.z);
        ...
        cam = (cam + camwidth) % 360;
        if( rot == 0) { rot = Math.PI / 2; }
}

if( this.cameracount > 1 ) {
        this.lidarCamera.rotateY( rot );
        //this.lidarCamera.lookAt(camera.position.x, camera.position.y, camera.position.z + 1);
        this.lidarCamera.updateProjectionMatrix();
        this.renderTarget.viewport.set(cam, 0, this.lidarWidth, this.rtargetHeight);
        this.renderTarget.scissor.set(cam, 0, this.lidarWidth, this.rtargetHeight);
        renderer.setRenderTarget(this.renderTarget);
...
//        this.lidarCamera.lookAt(camera.position.x, camera.position.y, camera.position.z - 1);
        ...
        if(ocean) ocean.visible = saveocean;
        if(gridhelper) gridhelper.visible = savegridhelper;
