...
local libs = ffi_wv_lib or {
   Windows = { x64 = "bin/windows/x64/webview.dll" },
   OSX     = { x64 = "bin/osx/webview.dylib" },
   Linux   = { x64 = "bin/linux/x64/webview.so", arm = "webview" },
}
...
ffi.cdef[[
    ...
    void webview_set_virtual_hostname(webview_t w, const char *hostname, const char *datapath);

    void webview_handle_scheme(webview_t wv, const char* scheme, WebResourceRequestedCallback reqfunction );

    void webview_install_handlers(webview_t w);
